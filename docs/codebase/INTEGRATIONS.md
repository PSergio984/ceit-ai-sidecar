...
| Corpus files | Filesystem hand-off | Reads `catalog.json` + `policies.json` from `CORPUS_PATH` (Laravel `ai:export-corpus` output) | n/a (files only, D-17) | High | `app/ingest.py` `load_documents` |
