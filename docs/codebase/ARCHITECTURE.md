...
rebuild:  /index/rebuild -> load_documents (validate) -> embed (ONNX fastembed)
...
- **Vector search IS used**: whole-document embeddings from `fastembed` (ONNX, `all-MiniLM-L6-v2`), normalized, cosine similarity via matmul (`vectors @ q`). Source: `app/search.py` `_semantic_scores`, `app/ingest.py` `embed_documents`.
