...
def load_documents(corpus_path: Path) -> list[dict]:
    ...
    for filename in INDEX_FILES:
        ...
        if payload.get("schema_version") != 1:
            raise ValueError(f"{filename}: schema_version must be 1, got {payload.get('schema_version')!r}")
        ...
        try:
            ...
            raise ValueError(f"{filename}: 'generated_at' is not a parseable ISO timestamp") from exc

        for doc in payload.get("documents", []):
            for field in ("id", "corpus", "title", "text", "metadata"):
                if field not in doc:
                    raise ValueError(f"{filename}: document missing required field '{field}': {doc.get('id')!r}")
    ...
    if found == 0:
        raise ValueError(f"no corpus files found in {corpus_path} (expected catalog.json and/or policies.json)")
...
def write_cache(cache_dir: Path, version: int, docs: list[dict], vectors: np.ndarray, db_path: Path) -> None:
