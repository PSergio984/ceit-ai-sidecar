...
def evaluate_case(engine: HybridSearch, case: dict, limit: int) -> dict:
    ...
    if case.get("negative"):
        ...
        return {"query": case["query"], "negative": True, "passed": passed,
...
def category_of(case: dict) -> str:
    ...
    if q.startswith("ceit-"):
        return "catalog_code"
    if "?" in case["query"] or "ba" in q.split() or "ano" in q.split() or "yung" in q.split() or "ko" in q.split():
        return "taglish"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Golden-set evaluation for the CEIT hybrid search sidecar")
    ...
    report = {
        ...
        "top1_rate": round(sum(1 for r in positives if r["top1_hit"]) / len(positives), 4) if positives else None,
        "overall": {
            "avg_precision": round(sum(r["precision"] for r in positives) / len(positives), 4) if positives else None,
            "avg_recall": round(sum(r["recall"] for r in positives) / len(positives), 4) if positives else None,
            "avg_f1": round(sum(r["f1"] for r in positives) / len(positives), 4) if positives else None,
        },
        "by_category": {
            cat: {
                "count": len(items),
    ...
    print(f"Golden set: {golden.get('catalog_snapshot')} | limit={args.limit} | {len(results)} cases")
    for result in results:
        if result["negative"]:
            status = "PASS" if result["passed"] else "FAIL"
            print(f"  [neg] {status}  {result['query'][:60]}")
        else:
            print(f"  p={result['precision']:.2f} r={result['recall']:.2f} f1={result['f1']:.2f}  {result['query'][:60]}")
    print(f"Negative pass rate: {report['negative_pass_rate']}")
    print(f"Top-1 rate: {report['top1_rate']}")
    print(f"Overall avg: P@{args.limit}={report['overall']['avg_precision']} "
          f"R@{args.limit}={report['overall']['avg_recall']} F1={report['overall']['avg_f1']}")
    for cat, stats in report["by_category"].items():
        print(f"  {cat}: n={stats['count']} P@{args.limit}={stats['avg_precision']} "
